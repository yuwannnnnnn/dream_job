...
if (isset($_POST['insertApplicationBtn'])) {
	...
	if (!empty($firstName) && !empty($lastName) && !empty($gender) && !empty($birthDate) && !empty($phoneNumber)  && !empty($email)  && !empty($roles) && !empty($experience)) {
			$query = insertIntoRecords($pdo, $firstName, $lastName, 
			$gender, $birthDate, $phoneNumber, $email, $roles, $experience);
